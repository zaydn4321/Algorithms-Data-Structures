...
public class Cipher {
	 ...
	 * for letter, this method is casting to interger to compare to the number of
	 * ASCII.
	 * the code checks if the letter is in alphabet or not and decide whether it
	 * should kept in same place or substitued.
	 */
	 ...
	 * ASCII.
	 * the code checks if the letter is in alphabet or not and decide whether it
	 * should kept in same place or reverse substitued letter
	 */
	public void decryption(String message_d) {
		...
		for (int i = 0; i < message_d.length(); i++) // loop through contents of the file.
		{
			...
			} else // if the letter is alphabet
			{
				...
				for (int j = 0; j < alphabet.length(); j++) {
					if (message_d.charAt(i) == Keyword.charAt(j)) // if the letter of keyphrase is identical to the one of the
																												// file
					{
	 ...
	 * it also check if the keyphrase is valid and only contains Alphabet
	 * letters/space.
	 */
	public void cipherList() {
		...
		for (int i = 0; i < keys.length; i++) {// Checks if the keyphrase is valid and only contains Alphabet letters/spaces
			if ((keys[i] >= 'a' && keys[i] <= 'z') || (keys[i] >= 'A' && keys[i] <= 'Z') || keys[i] == ' ') {
				...
				System.out.println("The keyphrase is not valid, you must enter English alphabetical letters, or a space.");
		...
		for (int i = 0; i < key.length(); i++) {
			...
			for (int j = 0; j < key.length(); j++) {
				...
				(key.charAt(i) == key.charAt(j))
					; // assign new letter if the letter are not the same.
	...
	public void readFile() {
		...
		while (ask) // keeps asking user for file name until they give a valid one
		{
			...
			File file = new File(filename); // file object is generated and the file is passed as an argument for
																			// constructor
			if (file.exists()) {
	...
	public void generateEncryptedNewFile() {
		try // use try~ catch block for exception.
		{
			...
			encryptedNew.write("E\n" + encryptText);// writes encrypted message to a new file named "EncryptNewFile.txt", with
																							// an 'E' to indicate it has been encrypted.
			encryptedNew.close();
